...
\begin{frame}[plain]
  \vspace{1.8cm}
  \begin{center}
    {\normalsize\color{darkgray} Course: Foundations of AI with Embeddings}\\[0.5cm]
    {\LARGE\bfseries Lecture 1: Basic dimensionality reduction algorithms}\\[0.4cm]
    \textcolor{accentred}{\rule{0.65\textwidth}{1.5pt}}\\[0.4cm]
    {\normalsize
      PCA \enspace$\cdot$\enspace
      MDS \enspace$\cdot$\enspace
      Local~MDS \enspace$\cdot$\enspace
      Isomap \enspace$\cdot$\enspace
      DBSCAN
    }\\[1.2cm]
    {\small Liubov Tupikina, Vladimir Kukushkin}
    \\[0.2cm]
    {\small Constructor University, 2026}
  \end{center}
\end{frame}
...
\begin{frame}[shrink=5]{Variance along an axis}
  \begin{columns}[T]
    ...
    \begin{tikzpicture}[scale=1.28]
      ...
      \draw[->, thick, darkgray] (0,0) -- (0.3,2.1);
...
\begin{frame}{A case study: handwritten digits}
  \begin{itemize}
      ...
      \item Each image is of $16\times 16 = 256$ pixels. Feature is a pixel's grayscale. 
  \end{itemize}
